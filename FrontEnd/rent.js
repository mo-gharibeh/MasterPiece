...
async function getProductsByCategoryId() {
    ...
    result.forEach(element => { // Iterate over the parsed JSON array
        ...
        <div class="col-lg-4 col-md-6 wow fadeInUp" data-wow-delay="0.3s">
            <div class="property-item rounded overflow-hidden">
                <div class="position-relative overflow-hidden">
                    <a onclick="saveProductId(${element.productId})"><img class="img-fluid" src="${element.imageUrl}" alt=""></a>
                    <div class="bg-primary rounded text-white position-absolute start-0 top-0 m-4 py-1 px-3">For ${element.productType}</div>
                    <div class="bg-white rounded-top text-primary position-absolute start-0 bottom-0 mx-4 pt-1 px-3">${element.category ? element.category.categoryName : "Unknown Category"}</div>
                </div>
                <div class="p-4 pb-0">
                    <h5 class="text-primary mb-3">${element.price} JOD</h5>
